<template>
  <div id="footer11">
    <div data-v-6de62122="" class="van-hairline--top-bottom van-tabbar van-tabbar--fixed">
      <div data-v-6de62122="" class="van-tabbar-item" :class="{active: isChoose[0]}" @click="getView(1)">
        <router-link to="/index" tag="span">
          <div class="van-tabbar-item__icon">
            <img data-v-6de62122="" v-if="isChoose[0]" src="data:image/png;base64,">
            <img data-v-6de62122="" v-if="isChoose[1]" src="data:image/png;base64," />
          </div>
          <div class="van-tabbar-item__text">
            <span data-v-6de62122="">首页</span>
          </div>
        </router-link>
      </div>
      <div data-v-6de62122="" class="van-tabbar-item" :class="{active: isChoose[2]}" @click="getView(2)">
        <router-link to="/classify" tag="span">
          <div class="van-tabbar-item__icon">
            <img data-v-6de62122="" v-if="isChoose[3]" src="data:image/png;base64,">
            <img data-v-6de62122="" v-if="isChoose[2]" src="data:image/png;base64,">
          </div>
          <div class="van-tabbar-item__text">
            <span data-v-6de62122="">分类</span>
          </div>
        </router-link>
      </div>
      <div data-v-6de62122="" class="van-tabbar-item" :class="{active: isChoose[4]}" @click="getView(3)">
        <router-link to="/shopping" tag="span">
          <div class="van-tabbar-item__icon">
            <img data-v-6de62122="" v-if="isChoose[5]" src="data:image/png;base64,">
            <img data-v-6de62122="" v-if="isChoose[4]" src="data:image/png;base64,">
            <div class="van-info">{{cartCount}}</div>
          </div>
          <div class="van-tabbar-item__text">
            <span data-v-6de62122="">购物车</span>
          </div>
        </router-link>
      </div>
      <div data-v-6de62122="" class="van-tabbar-item" :class="{active: isChoose[6]}" @click="getView(4)">
        <router-link to="/my" tag="span">
          <div class="van-tabbar-item__icon">
            <img data-v-6de62122="" v-if="isChoose[7]" src="data:image/png;base64,">
            <img data-v-6de62122="" v-if="isChoose[6]" src="data:image/png;base64," />
          </div>
          <div class="van-tabbar-item__text">
            <span data-v-6de62122="">我</span>
          </div>
        </router-link>
      </div>
    </div>
  </div>
</template>

<script>
  export default {
    name: 'footer11',
    props:['isChoose'],
    data() {
      return {
        view: 1,
        msg: 'footer page',
        cartCount: 0
      };
    },
    mounted() {
      this.$EventBus.$on("cartCount", (target) => {
        this.cartCount = target;
      });
    },
    methods: {
      getView(num) {
        switch (num) {
          case 1:
            {
              this.view = 1;
              break;
            }
          case 2:
            {
              this.view = 2;
              break;
            }
          case 3:
            {
              this.view = 3;
              break;
            }
          case 4:
            {
              this.view = 4;
              break;
            }
        }
      }
    }
  }
</script>

<style>
  #footer11 {
    position: fixed;
    bottom: 0;
    left: 0;
    right: 0;
    z-index: 299;
    padding: 10px 0;
    width: 100%;
    background-color: #fff;
  }

  .van-hairline--top-bottom.van-tabbar.van-tabbar--fixed {
    display: flex;
    z-index: 100;
    justify-content: space-around;
  }

  .van-tabbar-item {
    width: 25%;
    color: rgb(148, 148, 151);
    text-align: center;
  }

  .van-tabbar-item.active {
    color: rgb(254, 79, 112);
  }

  .van-tabbar-item.van-tabbar-item--active {
    color: rgb(254, 79, 112);
  }

  .van-tabbar-item__icon {
    position: relative;
  }

  .van-info {
    position: absolute;
    top: -2px;
    right: 18px;
    width: 20px;
    height: 20px;
    border-radius: 50%;
    color: #fff;
    background-color: rgb(254, 79, 112);
    font-size: 12px;
  }
</style>
